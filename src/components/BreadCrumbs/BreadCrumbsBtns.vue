<template>
  <v-flex xs12>
    <v-toolbar>
        ...
        >
          <router-link :to="item.to" tag="div">
            <div v-if="!item.disabled">
              <div :class="breadColor(subject)">{{ item.text }}</div>
            </div>
            <div v-if="item.disabled">{{ item.text }}</div>
          </router-link>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
      <v-menu bottom class="hidden-sm-and-up">
        <v-btn icon slot="activator"> <v-icon>list</v-icon> </v-btn>
        <v-list>
          <v-list-tile
            v-for="(item, i) in breadItems"
            :key="i"
      ...
      <v-menu bottom class="hidden-md-and-up">
        <v-btn icon slot="activator"> <v-icon>more_horiz</v-icon> </v-btn>
        <v-list>
          <v-list-tile
            v-for="(btn, i) in btns"
            :key="i"
